...
int config_init(char *path)
{
	...
	while(!feof(fd))
	{
		...
		if( !memcmp(buff, "collect_shell_name", strlen("collect_shell_name")))
		{
			sscanf(buff, "%s = %s",tmp1,collect_shell_name); 
			continue;
		}
	...
	|| user_name[0] == '\0' || mysql_db[0] == '\0' || collect_table_dir[0]=='\0' || collect_shell_name[0] =='\0' )
	{
		printf("config file is error\n");
		return 1;
	}
	printf("collect_act_dir    : %s\n", collect_act_dir);
	printf("collect_shell_name : %s\n", collect_shell_name);
