...
u32 (*cw_ap_sendto_ac_info)(dcma_udp_skb_info_t *buff)=NULL;
//cw_ap_sendto_ac_info = NULL;
...
{  
	switch(buff->type)  
	{
		...
		case CW_NLMSG_PUT_KLOG_INFO: 
			...
			if( NULL != cw_ap_sendto_ac_info ){
				cw_ap_sendto_ac_info(buff);
			}
			break;
        default:  
			break;
...
void *cloud_wlan_recv_kmod_info(void *param)
{
	s32 ret=0;
