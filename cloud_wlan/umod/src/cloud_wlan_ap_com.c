...
#include "cloud_wlan_db_struct.h"
...
u32 cw_cfg_dynamic_update_init()
{
	...
	switch(g_config_base.usage_model)
	{
		case USAGE_MODEL_ONLINE:
			cw_online_dynamic_update_init(&g_pthread);
			break;
		case USAGE_MODEL_LOCAL:
			cw_local_dynamic_update_init(&g_pthread);
			break;
		default:
			break;
	}
...
int main(int argc, char **argv)  
{
	...
	/* 本地默认一些配置的初初化*/
	cw_ap_local_db_cfg_init(g_cw_db);
